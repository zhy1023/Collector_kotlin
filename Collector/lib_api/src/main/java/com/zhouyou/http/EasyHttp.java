...
//            okHttpClientBuilder.addInterceptor(loggingInterceptor);
            addLogingInterceptor(loggingInterceptor);
        }
    ...
    /**
     * 添加全局拦截器位于Header拦截器之后
     */
    public EasyHttp addHeaderInterceptor(Interceptor interceptor) {
        if (null == headerInterceptor) headerInterceptor = new ArrayList<>();
        headerInterceptor.add(interceptor);
        return this;
    }
     ...
     * 添加日志拦截器
     */
    public EasyHttp addLogingInterceptor(Interceptor interceptor) {
    ...
    public static Interceptor getLogInterceptor() {
        return getInstance().logInterceptor;
    }
    ...
    public static List<Interceptor> getHeaderInterceptor() {
        return getInstance().headerInterceptor;
    }
    ...
    public EasyHttp addNetworkInterceptor(Interceptor interceptor) {
        okHttpClientBuilder.addNetworkInterceptor(Utils.checkNotNull(interceptor, "logInterceptor == null"));
        return this;
    }
