...
public abstract class BaseRequest<R extends BaseRequest> {
    ...
    private OkHttpClient.Builder generateOkClient() {
        if (readTimeOut <= 0 && writeTimeOut <= 0 && connectTimeout <= 0 && sslParams == null
            ...
            List<Interceptor> headerInterceptor = EasyHttp.getHeaderInterceptor();
            if (headerInterceptor != null && headerInterceptor.size() > 0) {
                for (Interceptor interceptor : headerInterceptor) {
            ...
            if (null != EasyHttp.getLogInterceptor())
                newClientBuilder.addInterceptor(EasyHttp.getLogInterceptor());
