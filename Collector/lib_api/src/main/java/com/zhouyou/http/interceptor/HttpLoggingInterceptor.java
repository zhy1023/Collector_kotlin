...
public class HttpLoggingInterceptor implements Interceptor {
    ...
    private void logForRequest(Request request, Connection connection) throws IOException {
        ...
        try {
            String requestStartMessage = "--> " + request.method() + ' '
                    + URLDecoder.decode(request.url().url().toString(), UTF8.name()) + ' ' + protocol;
    ...
    private Response logForResponse(Response response, long tookMs) {
        ...
        try {
            log("<-- " + clone.code() + ' ' + clone.message() + ' ' + URLDecoder.decode(clone.request().url().url().toString(), UTF8.name())
                    + " (" + tookMs + "ms）");
            if (logHeaders) {
                log(" ");
                ...
                for (int j = 0; j < requestHeaders.size(); j++) {
                    log("\t" + requestHeaders.name(j) + ": " + requestHeaders.value(j));
                }
